        ...
        switch (view.getId()){
            ...
            case R.id.cadastro_btn_cadastro:
                ...
                if (TextUtils.isEmpty(username))
                    Toast.makeText(Cadastro.this, getResources().getString(R.string.enter_username), Toast.LENGTH_SHORT).show();
                else if (!isValidEmail(email))
                    Toast.makeText(Cadastro.this, getResources().getString(R.string.email_nao_valido), Toast.LENGTH_SHORT).show();
                else if (!password.equals(confirma_password))
                    Toast.makeText(Cadastro.this, getResources().getString(R.string.password_nao_confere), Toast.LENGTH_SHORT).show();
                else if (acesso==null)
                    Toast.makeText(Cadastro.this, getResources().getString(R.string.seleciona_tipo_usuario), Toast.LENGTH_SHORT).show();
                else
                    Cadastrar();
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if (sucesso.equals("1")){
                        startActivity(new Intent(Cadastro.this, Login.class));
                        finish();
                        Toast.makeText(Cadastro.this, "Cadastro com  sucesso! Fazer  o login", Toast.LENGTH_SHORT).show();
                    }
                }catch (JSONException e){
                    e.printStackTrace();
                    Toast.makeText(Cadastro.this, "Cadastro sem sucesso! " +e.toString(), Toast.LENGTH_SHORT).show();
                }
            }
        },  new Response.ErrorListener(){
            @Override
            public void onErrorResponse(VolleyError error){
                Toast.makeText(Cadastro.this, "Conexão erro! " +error.toString(), Toast.LENGTH_SHORT).show();
            }
        })
        {
            @Override
